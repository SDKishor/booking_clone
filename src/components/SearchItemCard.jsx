...
export const SearchItemCard = ({ item }) => {
  const navigate = useNavigate();
  const handleHotelButton = (id) => {
    navigate(`/hotel/${id}`);
  };
  return (
    <div className="flex p-5 border-solid border-primary/20 border-4 rounded-md mb-5">
      ...
      <div className="flex-[2] px-3">
        <h1 className="font-bold text-2xl text-secondary pb-3">{item.name}</h1>
        <div className="font-bold pb-2">{item.distance}m from center</div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="">
          <div className="text-right font-bold text-2xl mb-3">
            ${item.cheapestPrice}
          </div>
          ...
          <button
            ...
            onClick={() => handleHotelButton(item._id)}
          >
            See availability
          </button>
        </div>
